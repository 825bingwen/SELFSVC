...
comment on column SH_ALARM_BIND.OPERID is '接受告警人';
...
comment on column SH_CHARGE_LOG.CHECKFLAG is '仅对银联卡缴费记录有用，如果记录经过对账，值为1(表明银联扣款成功)；否则，为空。取对账记录时，仅取checkflag=1的记录';
...
;
comment on table SH_INFO_REGIONLIST is '	区域信息表';
comment on column SH_INFO_REGIONLIST.REGION is '区域';
comment on column SH_INFO_REGIONLIST.REGIONNAME is '区域名称';
comment on column SH_INFO_REGIONLIST.ORGID is '所属机构';
...
comment on column SH_LOG_AUDIT.OPERID1 is '操作员ID1';
comment on column SH_LOG_AUDIT.OPERID2 is '操作员ID2';
