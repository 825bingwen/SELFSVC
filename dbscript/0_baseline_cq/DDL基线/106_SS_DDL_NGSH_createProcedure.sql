...
create or replace procedure PROC_SH_UPDATEALARMINFO2(v_minutes in number) is
...
v_count number;
begin
  for cl in (select a.alarminfoid, a.termid, a.exceptiontype from sh_alarm_info a where exists(select 1 from sh_cashfile_log t, sh_term_config c where c.ipaddr=t.ipaddr and c.termid=a.termid and t.recdate > sysdate - v_minutes / (24 * 60)) and exceptiontype='400' and unchain = 0) loop
        --在告警规则表中查询该告警类型对应的记录
...
begin
  ...
  --select * from sh_alarm_info where EXCEPTIONTYPE != '400' AND EXCEPTIONTYPE NOT IN (select status from sh_log_termstatus where ipaddr=v_ipaddr and statusdate > sysdate - 5 / (24 * 60)) AND TERMID = v_termid AND UNCHAIN = v_status
  --取终端机尚未解除的告警记录(不包括400)，但该告警实际已不存在(比如之前有301告警，但最近一次上传了0000，说明301已经实际解除了)
  --对上述查询出来的记录进行循环处理
  for cl in (select * from sh_alarm_info where EXCEPTIONTYPE != '400' AND EXCEPTIONTYPE NOT IN (select status from sh_log_termstatus where ipaddr=v_ipaddr and statusdate > sysdate - 5 / (24 * 60)) AND TERMID = v_termid AND UNCHAIN = v_status) loop
        --在告警规则表中查询该告警类型对应的记录
        select count(*) into v_count from sh_alarm_rule where exceptiontype=cl.exceptiontype and status = 1;
        --如果有，取告警持续时间的分隔线
        if v_count > 0 then
