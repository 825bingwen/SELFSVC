				...
				<div class="bg bg712"></div>
 					...
 					<div class="blank10"></div>
   					...
   					<div class="popup_confirm" id="openWin_tipsMsg">
						...
						<div class="tc">
							<a  href="javascript:void(0)" class=" bt4" onmousedown="this.className='bt4on';" onmouseup="this.className='bt4';windowClose();">继续缴费(确认键)</a> 
    						<a  class=" bt4 ml20" onmousedown="this.className='bt4on ml20'" onmouseup="this.className='bt4 ml20';closeRec();">取消办理(清除键)</a>
						</div>
					</div>
   				</div>
			</div>
...
function loadContent() 
{
    ...
    try 
    {
        ...
        else
        {
            ...
            //初始化成功，就需要关闭识币器。而首页、上一页、退出按钮无法执行此操作，所以禁用”首页“、“上一页”、“退出”按钮
...
{
    writeDetailLog("<%=(String) PublicCache.getInstance().getCachedData(Constants.SH_CHARGELOG_DETAILFLAG) %>", 
            "<s:property value='telnum' />取消开户交费交易");
