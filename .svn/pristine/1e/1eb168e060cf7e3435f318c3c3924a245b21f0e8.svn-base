		...
		<script language="javascript">
			...
			function goback(menuid)
			{
				if (submitFlag == 0)
				{
					...
					document.dutyInfoForm.action = "${sessionScope.basePath }agentCharge/agentChargePage.action";
			...
			function doSub() 
			{
				if (submitFlag == 0) 
				{
					...
					document.dutyInfoForm.target = "_self";
					document.dutyInfoForm.action = "${sessionScope.basePath }agentCharge/readAgentCard.action";
					document.dutyInfoForm.submit();
				}
			}
			...
			{			
				if (submitFlag == 0)
				{
					...
					//异常处理，只要出现了异常就要记录日志  
					document.dutyInfoForm.target = "_self";
					document.dutyInfoForm.action = "${sessionScope.basePath }agentCharge/goCardError.action";
					document.dutyInfoForm.submit();
				}			
			}	
