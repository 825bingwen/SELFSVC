...
<script type="text/javascript">
	...
	function selectTelNum(telnum, payfee, orgid, lowConsumFee)
	{
		...
		document.getElementById('chooseNum').style.visibility='hidden';
		document.getElementById('chooseNum').innerHTML = '<p class="fs16 tc">' + telnum + '</p><p class="fs14 tc">选号费：' + payfee + ' </p><p class="fs14 tc">月最低消费：' + lowConsumFee + ' </p>';
		document.getElementById('chooseNum').style.visibility='visible';			
		
		document.getElementById('winText').innerHTML = '<span class="yellow">您选择开户：</span><br />号码：' + telnum + '<br />选号费：' + payfee + '<br />月最低消费：' + lowConsumFee;
	}
	...
	function gotoPrintSuccess()
	{
		...
		// 校验空白卡信息
		document.actform.action="${sessionScope.basePath }cardInstall/chkBlankCardInfo.action";
		document.actform.submit();
	}
</script>
