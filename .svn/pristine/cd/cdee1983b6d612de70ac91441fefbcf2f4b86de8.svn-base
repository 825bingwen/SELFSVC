...
<%
	...
	String newflow = (String)request.getAttribute("newflow");
...
function btnClick(btn,btClass,month)
{
	...
	if (submitFlag == 0)
	{
		...
		var effectMonty = <%=(String)PublicCache.getInstance().getCachedData("SH_ACCOUNT_EFFECTMONTH") %>; 
		...
		%>		
			...
			document.actform.action = "${sessionScope.basePath}nxfeeservice/queryMonthBill.action";
		<%
		}
		else
		{
		%>
			if (month < effectMonty)
			{			
				document.actform.action = "${sessionScope.basePath}nxfeeservice/queryMonthBill.action";
			}
			else
			{
				document.actform.action = "${sessionScope.basePath}nxfeeservice/qryBillCycle.action";			
			}
		<%
		}
		%>
