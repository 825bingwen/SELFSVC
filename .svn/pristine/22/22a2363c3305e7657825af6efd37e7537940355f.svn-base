...
<html>
	<head>
		...
		function updateAndCommit(resultstr)
		{
			...
			params1 = params1 + "&status=01";//交易状态 01表示银联扣款成功的临时状态
		...
		function doCardTrans()
		{
			...
			try
			{
				...
				//102：代理商交费，101：个人交费
				//传给银联的附加信息:交费类型（3）+交费日志唯一标识（14）+代理商交费前流水号
				strin = strin + formatStr("102"+document.getElementById("chargeLogOID").value+document.getElementById("beforeChargeNo").value,'right',' ',100);// 移动缴费附加信息100
		...
		function getOrderNo()
		{
			...
			var loader1 = new net.ContentLoader(url1, netload = function () 
			{
				...
				{	
					setException("交费前信息记录失败，银联扣款失败，无法充值，请联系营业人员取走您的银联卡。");
					return;
				}								
			...
			<div class="main" id="main">
				...
				<div class="pl30">
					<div class="b257 " id="flowDesc1">
    					...
    					<a href="javascript:void(0)">2. 选择金额</a> 
