...
// add end qWX279398 2015-08-25 OR_HUB_201508_599 关于整改自助缴费机安全漏洞的需求
	...
	String prepareTip = CommonUtil.getParamValue("SH_PREPARECARD_TIP");
	if(prepareTip == null)
	{
		prepareTip = "备卡适用于原(U)SIM卡正常可用，客户想更换新(U)SIM卡以便适用于新手机的情况。";
	}
...
<html xmlns="http://www.w3.org/1999/xhtml">
			...
			if (submitFlag == 0) 
			{
				...
				// 现金识币器有问题，银联读卡器可用
				else if (chkCashMsg != "" && chkCardMsg == "")
			 	{
			 		...
			 		openRealNameConfirm("现金识币器故障，本次备卡只能用银联卡备卡，是否继续？", "现金识币器异常，用户已取消操作");
			 	}
			 	// 现金识币器可用，银联读卡器有问题
			 	else if (chkCashMsg == "" && chkCardMsg != "")
			 	{
			 		...
			 		openRealNameConfirm("银联读卡器故障，本次备卡只能用现金备卡，是否继续？", "银联读卡器异常，用户已取消操作");
			 	}
				else
				{
					...
					document.actform.action = "${sessionScope.basePath }prepareCard/validPassword.action";
	...
	<body scroll="no" onload="getObj('numBoardText1').focus();doLoad();">
		<form name="actform" method="post">	
			...
			<%-- 是否跳转营销推荐活动页面 --%>
			<input type="hidden" id="toRecActFlag" name="toRecActFlag" value='' />
			...
			<%-- 备卡提示 --%>
			<input type="hidden" id="prepareTip" name="prepareTip" value="<%=prepareTip %>"/>
		...
		{
			// 备卡提示 终端机支持空白卡类写卡型提示信息
			alertSuccessMsg(getValue("prepareTip")+"<br/>"+"<s:property value="blankCardTypeAlertMsg"/>");
		}
	</script>
</html>
