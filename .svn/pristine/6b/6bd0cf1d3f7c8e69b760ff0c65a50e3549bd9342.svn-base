		...
		if (dataArray[0] == "1") 
		{
			...
			**/
			...
			try
			{
				...
				bankrequest = bankrequest + formatStr('<s:property value="telnum" />','right',' ',20);// 电话号码(20)
				...
				{
					// 成功
					bankresponse = '00    4563518600005509778 001778000000005000交易成功                                09510000000000100000002      1126164654000001217097          ' + randomNumber;
			...
			try
			{
				writeDetailLog("<%=chargelog_detailflag %>", "银联卡交易完成后收尾工作...");
			...
			if (bankresponse.substring(0,2) == "00" && strlen(bankresponse) == 148 && bankresponse.substring(bankresponse.length-3,bankresponse.length) == randomNumber)
			{
				...
				params1 = params1 + "&description=" + encodeURI(encodeURI('开户之前扣款并缴费成功'));
