...
<html>
	<head>
		...
		function doSub() 
		{
			...
			if (submitFlag == 0) 
			{
				...
				document.actform.action = "${sessionScope.basePath}prodInstall/telnumResult.action";
	...
	<body onload="focusObj();" scroll="no">
						...
						<div class="custom_money pl30">
							<s:if test="#request.sele_rule == 2">
								<span class="pl40 fs20 fl lh48">前缀：</span>
          						<input type="text" id="tel_prefix" name="tel_prefix" maxlength="7" class="text1 fl" onclick="MoveLast(event);"/>
							</s:if>
							<s:elseif test="#request.sele_rule == 3">
								<span class="pl40 fs20 fl lh48">后缀：</span>
          						<input type="text" id="tel_suffix" name="tel_suffix" maxlength="4" class="text1 fl" onclick="MoveLast(event);"/>
							</s:elseif>							
       					</div>
		...
		<script type="text/javascript">
		// 随机方式
		if ("4" == "<s:property value='sele_rule'/>")
		{	
			// 提交
			doSub();
		}
