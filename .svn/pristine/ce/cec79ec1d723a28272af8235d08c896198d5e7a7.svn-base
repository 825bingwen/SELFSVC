...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function doSub()
		{
			...
			if (startDate < '<s:property value="farthestDate" />')
			{
				changObj(document.getElementById('numBoardText1'), 1);
				alertRedErrorMsg("系统仅允许查询6个月之内的详单信息，开始日期不可早于<s:property value='farthestDate' />");
				return;
			}
			else if (startDate.substring(0, 6) != endDate.substring(0, 6))
			{
				changObj(document.getElementById('numBoardText1'), 1);
				alertRedErrorMsg("不允许跨月查询");
				return;
			}
			else if (endDate < startDate)
			{
				changObj(document.getElementById('numBoardText2'), 2);
				alertRedErrorMsg("开始日期应小于结束日期");
				return;
			}
	...
	<body scroll="no" onload="document.getElementById('numBoardText1').focus();moveLast(document.getElementById('numBoardText1'));">
						...
						<div class="keyboard_wrap clearfix">
							<ul class="phone_num_list fl">
          						<li class="on fs20 clearfix" id="phone_input_1" >
            						...
            						<input type="text" maxlength="8" id="numBoardText1" name="startDate" class="text1 fl relative" onclick="moveLast(this);changObj(this, 1);" value="<s:property value='startDate' />" />
          						</li>
          						<li class="fs20 clearfix" id="phone_input_2">
          							<i class="lh30">2.请输入结束日期(格式：20110101)</i>
          							<span class="pl20 fl lh75">结束日期：</span>
            						<input type="text" maxlength="8" id="numBoardText2" name="endDate" class="text1 fl relative" onclick="moveLast(this);changObj(this, 2);" value="<s:property value='endDate' />" />
         						</li>         
        					</ul>
