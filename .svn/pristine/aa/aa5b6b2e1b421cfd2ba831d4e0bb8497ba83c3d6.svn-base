...
function goback(menuid)
{
	if (submitFlag == 0)
	{
		...
		<%
		if ("HUB".equals((String)PublicCache.getInstance().getCachedData(Constants.PROVINCE_ID)))
	    { 
		%>
			document.getElementById("curMenuId").value = "recHotspot";
			document.forms[0].action = "${sessionScope.basePath }reception/recHotspot_hub.action";
		<%
		}
		else
		{
