...
function goback(menuid)
{
	...
	if (submitFlag == 0)
	{
		...
		document.actform.action = "${sessionScope.basePath }broadBandPay/qryfeeChargeAccount.action";
...
function doSub()
{
	...
	if (submitFlag == 0)
	{
		...
		//提交缴费请求
		document.actform.target = "_self";
		document.actform.action = "${sessionScope.basePath }broadBandPay/cashChargeCommit.action";
		document.actform.submit();
	}
}
...
{			
	...
	// 异常处理，只要出现了异常就要记录日志  
	document.actform.target = "_self";
	document.actform.action = "${sessionScope.basePath }broadBandPay/goCashError.action";
	document.actform.submit();
}		
