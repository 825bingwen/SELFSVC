...
{		
	...
	if (submitFlag == 0) 
	{
		...
		document.readPwdForm.action = "${sessionScope.basePath }broadBandPay/goCardError.action";
...
function doSub() 
{
	if (submitFlag == 0) 
	{
		...
		// 执行提交
		document.readPwdForm.target = "_self";
		document.readPwdForm.action = "${sessionScope.basePath }broadBandPay/toMakeSure.action";
		document.readPwdForm.submit();
	}
}
