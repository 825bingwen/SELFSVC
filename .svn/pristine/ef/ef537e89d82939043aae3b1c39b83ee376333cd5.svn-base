...
.tb_blue_08{border:1px solid #1a9ae3; border-collapse:collapse; font-size:20px;height:30px;}
.tb_blue_08 td ,.tb_num{ border:1px solid #1a9ae3; border-collapse:collapse; font-size:20px; color:#fff; height:30px;}
.tb_blue_08 th{ background:#0b8ac1; border:1px solid #1a9ae3; border-collapse:collapse; font-size:20px; color:#fff; height:30px;}		
		</style>
  </head>
  
  <body onload="qryCurBillComm();">
  		<table id="totalTab" border="0" cellpadding="0" cellspacing="0">
  			  <p class="tit_info" align="left">通信量总量</p>
		      <table id="curBcommTab" width="100%" class="tb_blue_08" >
					<tr>
			 ...
			 <table id="curBcommTab2" width="100%" class="tb_blue_08" >
			 </table>
			 <p class="tit_info" align="left">套餐内通信量明细</p>
			 <table id="curBcommTab3" width="100%" class="tb_blue_08" >
			 </table>
		</table>
	 <script type="text/javascript">
	 		...
	 		function changeHeight(hei)
			{
				try
				{
					parent.document.getElementById("ifr_comm").style.height = hei;
				}
				catch(e1)
				{
				}
			...
			function qryCurBillComm()
			{
				var url = "${sessionScope.basePath }hubfeeservice/curBillCommDetail.action";
			...
			function displayBillComm(xmlData)
			{
				...
				var _height=0;
				if(_size>0){
					var tab = document.getElementById('curBcommTab');
		  	 		tab.deleteRow();
				...
				var _size = _oNodes.length;				
				if(_size>0){
					var tab = document.getElementById('curBcommTab2');
		  	 		//tab.deleteRow();
				...
				var _size = _oNodes.length;
				if(_size>0){
					var tab = document.getElementById('curBcommTab3');
		  	 		//tab.deleteRow();
				...
				parent.document.all("ifr_comm").style.height=(_height+70);
				changeStartPrint();
			}
