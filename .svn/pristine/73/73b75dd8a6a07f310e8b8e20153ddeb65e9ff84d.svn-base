...
String fenMoney = CommonUtil.yuanToFen(fee);
%>
	...
	<head>
		...
		************************************************/
		...
		function setException(errorMsg) 
		{
			...
			document.actForm.action = "${sessionScope.basePath }broadBandPay/goCardError.action?errPosResCode=" + document.getElementById('posResCode').value;
		...
		function payToBoss() 
		{
			if (document.getElementById("fee").value == "" 
					|| parseInt(document.getElementById("fee").value) <= 0)
			{
				return;
			}
			...
			document.actForm.target = "_self";
			//modified by xkf57421 for XQ[2011]_10_082 begin
			document.actForm.action = "${sessionScope.basePath }broadBandPay/cardChargeCommit.action?cmtPosResCode=" + document.getElementById('posResCode').value;;
			//modified by xkf57421 for XQ[2011]_10_082 end
			document.actForm.submit();			
		}
		...
		function fPosPay()
		{
			try
			{
				...
				//扣款之前记录日志
				var url = "${sessionScope.basePath }broadBandPay/addChargeLog.action";
				
				var params = "servnumber=" + <s:property value='servnumber' /> + "&paytype=1&fee=" + <s:property value='fee' />;
				...
				var loader = new net.ContentLoader(url, netload = function () {
						...
						if (dataArray[0] == "1") 
						{
							...
							try
							{
								...
								**/	
								...
								if (resultstr.substring(0,2) == "00" && resultstr.length == 34)
								{
									...
									document.getElementById("fee").value = printcontexts[9];
									
									//更新日志
									var url1 = "${sessionScope.basePath }broadBandPay/updateCardChargeLog.action";
							...
							catch (e)
							{
								...
								if (document.getElementById("fee").value != "" && parseInt(document.getElementById("fee").value) > 0)
								{
			...
			<div class="main" id="main">
				...
				<div class="pl30">
					<div class="b257 ">
						...
						<a href="javascript:void(0)">1. 输入宽带账号</a> 
						<a href="javascript:void(0)">2. 选择宽带产品</a>
					...
					<div class="b712">
    					...
    					<div class="p40">
    						<p class=" tit_info"><span class="bg"></span>宽带账号：<span class="yellow"><s:property value="servnumber" /></span></p>
    						<p class="fs22 fwb pl40 lh30">交费金额：<span class="yellow fs22"><s:property value="fee" /></span> 元</p>
