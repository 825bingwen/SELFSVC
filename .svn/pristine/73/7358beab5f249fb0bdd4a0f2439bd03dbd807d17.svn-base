...
**/
function OpenComByCard()
{
	//return "0";
	var ret = "";
	try{
		ret = window.parent.document.getElementById("cardpluginid").OpenCom();
		return ret;
	}catch(e){
		return "-1";
	}
...
/**
功能：获取卡位置，以判断用户是否已插入银联卡
返回：0 用户已插入银联卡；-1 获取失败；1 用户尚未插入银联卡。
场景：平台系统调用OpenCom()接口正常打开读卡设备后，页面定时循环调用此接口获取卡位置。如果返回0，表示用户已插入银联卡，停止定时循环调用；如果返回-1，表示获取卡位置失败；如果返回1，表示用户尚未插入银联卡，继续定时循环调用。如果在指定时间内(30秒)用户一直未插入银联卡，则认为插卡超时，停止循环调用。
**/
function getCardPosition()
{
	...
	var ret = "";
	try{
		ret = window.parent.document.getElementById("cardpluginid").getCardPosition();
		return ret;
	}catch(e){
		return "-1";
	}
...
功能：关闭读卡设备，释放串口
返回：0 表示正常；-1 表示异常
场景：平台系统检测到用户已插入银联卡后，需调用此接口关闭读卡设备，释放串口。读卡设备后续交由第三方使用。
      平台系统调用OpenCom()接口正常打开读卡设备后，若用户在指定时间内一直未插入银联卡，则超时后也需调用此接口关闭读卡设备，释放串口。
**/
function CloseComByCard()
{
	//return "0";
	var ret = "";
	try{
		ret = window.parent.document.getElementById("cardpluginid").CloseCom();
		return ret;
	}catch(e){
		return "-1";
	}
...
功能：吞卡
返回：0 表示正常；-1 表示异常
场景：退卡后，若用户在指定时间内没有取走银联卡，则调用此接口吞掉用户银联卡。
**/
function CaptureBankCard()
{
	//return "0";
	var ret = "";
	try{
