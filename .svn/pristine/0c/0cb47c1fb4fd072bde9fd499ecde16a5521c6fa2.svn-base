...
openWindow = function(id,commendOID,actId,recmdType,actName,userSeq)
{
	...
	//{
		...
		// modify begin wWX217192 2014-08-13 OR_HUB_201406_225 精准营销二期
		document.getElementById("userSeq").value = userSeq;
		// modify end wWX217192 2014-08-13 OR_HUB_201406_225 精准营销二期
								...
								<p class="ptop180 tc p747w411h" id="inn" >
            						 <table width="100%" class="tb_blue" id="actTable">
					                  ...
					                  <tr>
					                    ...
					                    <th scope="col">业务介绍</th>
					                    <th style="width: 10%" scope="col">是否办理</th>
					                  </tr>
				                  	  ...
				                  	  <tr>
						                  ...
						                  <!-- <td><a href="javascript:recommendProduct('<s:property  value="#prod.commendOID"/>','<s:property  value="#prod.userSeq"/>');" id ="<s:property  value='#prod.commendOID'/>" name = "<s:property  value='#prod.userSeq'/>" class="bt6 fr relative fl ml20"  onmousedown="this.className='bt6on fl relative ml20'" onmouseup="this.className='bt6 fl relative ml20';">办理</a></td> -->
						             	  <td>
						             	  	<s:if test="%{#prod.isFeedBackDef == 1}">
						             	  		<a href="javascript:void(0);" id="<s:property value='#prod.commendOID'/>" 
						             	  			...
						             	  			onmouseup="this.className='bt6 fl relative';selFeedBackDef('<s:property value="#prod.commendOID"/>','<s:property value="#prod.actID"/>','<s:property value="#prod.commendType" />');">选择</a>
						             	  	</s:if>
						             	  	<s:elseif test="%{#prod.commendType != '-1'}">
						             	  		<a href="javascript:void(0);" id="<s:property value='#prod.commendOID'/>" 
						             	  			name="<s:property value='#prod.userSeq'/>" class="bt6 fr relative fl"
						             	  			onmousedown="this.className='bt6on fl relative'" 
						             	  			onmouseup="this.className='bt6 fl relative';openWindow('popup_confirm','<s:property value="#prod.commendOID"/>','<s:property value="#prod.actID"/>','<s:property value="#prod.commendType" />','<s:property value="#prod.actName" />','<s:property value="#prod.userSeq" />');">办理</a>
						             	  	</s:elseif>
						             	  </td>
						             </tr>
				                  	 </s:iterator>
