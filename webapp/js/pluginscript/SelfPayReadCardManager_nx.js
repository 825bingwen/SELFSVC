...
* 返回：0 成功；失败返回-1
* 场景：进入银联卡密码输入页面后使用
* @remark create cKF76106 2013/04/07 R003C13L03n01 银联密码框美化
*/
function CloseCom()
{
	//return "0";
...
function openPin()
{
	...
	try
	{
		ret = window.parent.document.getElementById("unionpluginid").openPin();
		return ret;
	}
	catch(e)
	{
...
* 功能：获取当前用户密码输入情况
* 返回：0x13 成功；失败返回-1
* 场景：调用准备输入密码接口（openPin()）后，定时循环调用此接口获取用户密码输入状态。
* @remark create cKF76106 2013/04/07 R003C13L03n01 银联密码框美化
*/
function getPass()
{
	...
	try
	{
		ret = window.parent.document.getElementById("unionpluginid").getPass();
...
function closePin()
{
	...
	try
	{
		ret = window.parent.document.getElementById("unionpluginid").closePin();
