...
function doSub() 
{
	...
	if (trim(randomCode) == "" || isNaN(randomCode) || pangu_getStrLen(trim(randomCode)) != 4) 
	{
	...
	//验证短信随机码
	var res = validRandomPwd();
	
	if (res == "-1") 
	{
		alertRedErrorMsg("短信验证码超时，请重新获取！");
		changeImage();
		return;
	}
...
// 校验短信随机码
function validRandomPwd()
{
	var url = "${sessionScope.basePath}prepareCard/validRandomPwd.action";
	var params = "randomPwd=" + getValue("randomPwd");
	var resXml;
	var loader = new net.ContentLoaderSynchro(url, netload = function () 
	{
		resXml = this.req.responseText;
