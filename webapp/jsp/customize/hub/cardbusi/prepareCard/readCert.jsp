...
function goback(menuid)
{
	if (submitFlag == 0)
	{
		...
		document.readCertForm.action = "${sessionScope.basePath }prepareCard/selectCertOrPwd.action";
		document.readCertForm.submit();
	}
}
...
{			
	if (submitFlag == 0)
	{
		...
		//异常处理，只要出现了异常就要记录日志  
		document.readCertForm.action = "${sessionScope.basePath }prepareCard/addRecLog.action";
		document.readCertForm.submit();
	}			
}
...
function doSub()
{
	if (submitFlag == 0)
	{
		...
		document.readCertForm.action = "${sessionScope.basePath}prepareCard/certShow.action";
		document.readCertForm.submit();	
	}			
}
...
function calLeftTime()
{
	...
	{           	
       	setException("操作超时!为保证您的信息安全请按\"退出\"返回首页,如需继续办理请按\"上一页\",谢谢使用!");
       	return;
	}
}
...
function getReadCardStatus() 
{
	...
	try 
	{
		...
		if (ret.substring(0,1) == "0" || ret.substring(0,1) == 0) 
		{
			...
			if(photo == "-1")
			{
				setException("获取身份证照片失败，无法使用身份证备卡。");
				return;
			}
			else
			{
		...
		{			
			setException("获取身份证文字内容失败，无法使用身份证备卡。");
			return;
		}
		else if (ret == "2")
		{		
			setException("证件无法识别，无法使用身份证备卡。");
			return;
		}
		else if (ret == "1")
		{		
			if(limitTime<=0)
			{
				setException("未读取到身份证信息，无法使用身份证备卡。");
				return;
			}
		} 
	}
	...
	{		
		setException("身份证读卡器读卡失败，无法使用身份证备卡。");
	}
}
...
{	
	...
	{
		setException("读卡器读卡失败，无法使用身份证备卡。");
	}
}
...
function getIdCardStatus()
{
	try
	{
		...
		if (ret != "0" && ret != 0) 
		{					
			setException("身份证读卡器状态异常，无法使用身份证备卡。");
			return;
		}
	}
	catch (excep) 
...
function bodyLoad() 
{
	...
	{				
		...
		if (ret != "0" && ret != 0) 
		{
			setException("初始化身份证读卡器异常，无法使用身份证备卡!");
            return;
		}
		else
		{		
...
<body scroll="no">
	<form name="readCertForm" method="post" target="_self">
		<!-- 错误信息 -->
