...
{	
	var pattern = /^\d{6}$/;
	var mgtpwd = document.getElementById("mgtpwd").value;
	if (mgtpwd == "" ||  !pattern.test(mgtpwd))
	{
		
		if ("1" == "<%=popupFlag %>")
		{
			alertRedErrorMsg("请输入六位管理员密码");
		}
		else
		{
			document.getElementById("errorMsg").innerHTML = "请输入六位管理员密码";				
		}
	...
	if (submitFlag == 0)
	{
		...
		document.actform.action="${sessionScope.basePath}rectelInfo/checkMgtpwd.action";
...
function goback()
{
	...
	<body scroll="no" onload="document.getElementById('mgtpwd').focus();">
					...
					<p class="fs22 mb30"></p>
						...
						<div class="keyboard_wrap clearfix">
							<ul class="phone_num_list fl">
          						<li class="on fs20 clearfix" id="mgtpwdInput" >
          							<i class="lh30">1.请输入管理员密码</i>
          							<span class="pl20 fl lh75">密&nbsp&nbsp码：</span>
            						<input type="password" maxlength="6" id="mgtpwd" name="mgtpwd" class="text1 fl relative" onclick="changObj(this, 1);" onfocus="MoveLast(event);"/>
         						</li>         
        					</ul>
        					<!--小键盘-->
							...
							var lastObj = document.getElementById('mgtpwd');
							var type = 1;
							...
							{
								lastObj = o;
								document.getElementById('mgtpwdInput').className = "on fs20 clearfix";
							}					
