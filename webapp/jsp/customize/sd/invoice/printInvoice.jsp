...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function printInvoice(invoice) 
			{
				...
				var url = "${sessionScope.basePath }charge/insertInvoiceLog.action?servnumber=<s:property value='servnumber' />&cycle=<s:property value='invoicePrint.billCycle' />";
        					...
        					<div class="line w625"></div>
       						...
       						<div class="recharge_result print tc">
          						...
          						<div class="btn_box4 clearfix">
          							<a href="javascript:void(0)" onmousedown="this.className='hover'" onmouseup="this.className=''" onclick="continuePrint('<s:property value='curMenuId' />');return false;">继续打印</a>     							
          						</div>
       						</div>
      					</div>
  					</div>
	...
	<script type="text/javascript">
		...
		// 继续打印
		function continuePrint(menuid)
		{	
			document.payMoneyForm.target = "_self";
			if(menuid == "recMonthInvoice")
			{
				document.payMoneyForm.action = "${sessionScope.basePath}monthInvoicePrint/qryCurrentMonth.action";
			}
			else
			{
				document.payMoneyForm.action = "${sessionScope.basePath}invoice/qryCurrentMonth.action";
			}
