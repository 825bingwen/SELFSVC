...
<html>
	...
	<body scroll="no" onload="window.focus();">
		<form name="actform" method="post">
			...
			<s:hidden name="cardNum"></s:hidden>
			
			<!-- begin and by gWX301560 2015-8-12 OR_SD_201506_971_山东_关于存量非实名制客户补登记相关系统支撑的需求  -->
			<s:hidden name="randomPwd"></s:hidden>
			<!-- end and by gWX301560 2015-8-12 OR_SD_201506_971_山东_关于存量非实名制客户补登记相关系统支撑的需求  -->
			
			<s:hidden id="selectMethod2" name="selectMethod2" value="selectServerPwd"></s:hidden>
							...
							<div class="keyboard_box">
								<div class="phone_num_list selectInput_list fl">
									<div class="fs20 textSelect clearfix ">
										...
										<div class="textSelect_con" id="checkcon"
											style="display: none">
											<span id="check3" onclick="showSelect('check',3)">服务密码</span>
											<span id="check2" onclick="showSelect('check',2)">充值记录</span>
											<span id="check1" onclick="showSelect('check',1)">通话记录</span>
										</div>
									</div>
								</div>
							...
							<div class="numboard numboard_big fl" id="numBoard">
								<div class=" numbox">
									...
									<div class="nleft">
										...
										<a id="pointOrX" href="javascript:void(0)">x</a><a href="javascript:void(0)">0</a><a href="javascript:void(0)">#</a>
									</div>
									<div class="nright">
...
// 提交标识
var submitFlag = 0;
var lastObj= document.getElementById('phone_input_pass_Text1');
var lastObj2=document.getElementById('phone_input_pass');
var T=1;
var O='phone_input_pass';
...
{
	var methodClicked = document.getElementById(m+'con');
	...
	if(n == 0)
	{
		if(methodClicked == '')
		{
			methodClicked.style.display="none";
		}
	   	else
	    {
	    	methodClicked.style.display="";
	    }
    }
    else
  	{
		...
		if("check1" == M)
		{
			...
			//begin and by gWX301560 2015-8-12 OR_SD_201506_971_山东_关于存量非实名制客户补登记相关系统支撑的需求
			document.getElementById('phone_input_pass_Text1').value='';
			//end and by gWX301560 2015-8-12 OR_SD_201506_971_山东_关于存量非实名制客户补登记相关系统支撑的需求
			...
			changObj(O,1);
			document.getElementById("selectMethod2").value="selectCallRecord";
		}
		// 充值记录
		else if("check2" == M)
		{
			...
			lastObj2=document.getElementById(O);
			changObj(O,1);
			document.getElementById("selectMethod2").value="selectRechargeRecord";
		}
		// 服务密码验证
		else if("check3" == M)
		{
			...
			lastObj2=document.getElementById(O);
			changObj(O,1);
			document.getElementById("selectMethod2").value="selectServerPwd";
		}
	}
} 
...
{
	// 认证方式
	var selectMethod2 =  document.getElementById("selectMethod2").value;
	
	var patternPwd = /^\d{6}$/;
	
	// 服务密码验证必须输入服务密码
	if("selectServerPwd" == selectMethod2)
	{
	...
	// 充值记录验证
	else if("selectRechargeRecord" == selectMethod2)
	{
	...
	// 通话记录验证
	else if("selectCallRecord" == selectMethod2)
	{
		var pattern = /^\d{11}$/;
...
function doSub()
{
	if (submitFlag == 0 && checkBeforeSub())
	{
		submitFlag = 1;
		openRecWaitLoading();
		document.actform.action = "${sessionScope.basePath}realNameReg/secondAuthMethods.action";
