		...
		<script type="text/javascript">
			...
			function doSub()
			{
				if (submitFlag == 0)
				{
					...
					document.readCardForm.target = "_self";
					document.readCardForm.action = "${sessionScope.basePath}cardInstall/certShow.action";
					document.readCardForm.submit();
				}			
			}
			...
			function getReadCardStatus() 
			{
				...
				try 
				{
					...
					if (ret.substring(0,1) == "0" || ret.substring(0,1) == 0) 
					{
						...
						if(photo == "-1")
						{
							setException("获取身份证照片失败，无法使用身份证开户。");
							return;
						}
						else
						{
					...
					{			
						setException("获取身份证文字内容失败，无法使用身份证开户。");
						return;
					}
					else if (ret == "2")
					{		
						setException("证件无法识别，无法使用身份证开户。");
						return;
					}
					else if (ret == "1")
					{		
						if(limitTime<=0)
						{
							setException("未读取到身份证信息，无法使用身份证开户。");
							return;
						}
					} 
				}
				...
				{		
					setException("身份证读卡器读卡失败，无法使用身份证开户。");
				}
			}
			...
			{	
				...
				{
					setException("读卡器读卡失败，无法使用身份证开户。");
				}
			}
			
			//获取身份证读卡器状态
			function getIdCardStatusInfo()
			{
				try
				{
					// 获取身份证读卡器状态
					...
					if (ret != "0" && ret != 0) 
					{					
						setException("身份证读卡器状态异常，无法使用身份证开户。");
						return;
					}
				}
				catch (excep) 
			...
			function bodyLoad() 
			{
				...
				{				
					...
					if (ret != "0" && ret != 0) 
					{
						setException("初始化身份证读卡器异常，无法使用身份证开户!");
			            return;
					}
					else
					{		
